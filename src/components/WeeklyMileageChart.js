...
// Custom dot with weekly total label above it
const CustomDot = (props) => {
  const { cx, cy, payload } = props;
  if (!payload.actual) return null;
  return (
    <g key={`dot-group-${cx}-${cy}`}>
      <circle
        cx={cx} cy={cy}
      ...
      <text
        ...
        fill="#7D2935"
        fontWeight={payload.isCurrent ? '500' : '400'}
      >
        {payload.actual}
      </text>
    </g>
...
export default function WeeklyMileageChart({ activities, plan }) {
  ...
  const withData = data.filter(d => d.actual > 0);
  const avgActual = withData.length
    ? Math.round(withData.reduce((s, d) => s + d.actual, 0) / withData.length * 10) / 10
    : 0;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          <div style={{ fontSize: '13px', fontWeight: 500, color: 'var(--navy)' }}>Weekly Mileage</div>
      ...
      <div style={{ display: 'flex', gap: '16px', marginTop: '12px' }}>
        {[
          ...
          { color: 'var(--navy)', label: 'Planned', opacity: 0.35 },
        ].map(l => (
